...
namespace StoreApp
{
    ...
    class OrderPrompt : IOrderPrompt
    {
        ...
        public bool Prompt()
        {
            ...
            NamePrompt();   //Promps user + get full name + save to DB
            ...
            while (true)
            {
                for (int i = 0; i < products.Length;)
                {
                    ...
                    {   //List<Location> availableStores //location Number == out storeNumber line 135
                        ...
                        if (!availableStores.Where(s => s.Number == storeNumber).FirstOrDefault()  //output store #(input)
                            .ValidateProduct(new Product(product), quantity))  
                        {
                        ...
                        Console.WriteLine("\nThe quantity can only be 1-{QUANTITY_LIMT}");
                }
                break;
            }
            return false;
